...
#use: aln_mouse.sh <FILE_NAME> <G_REF> <PRE_REF> <NUM_T> <date>
...
bwa aln -t ${NUM_T} -f results/aln/PE/${FILE_NAME}_R1_${PRE_REF}.sai  genome/mouse/${PRE_REF} rawdata/PE/${FILE_NAME}_R1.fastq.gz 1>> results/logs/PE/${LOG} 2>> results/logs/PE/${LOG}

#ALN R2

bwa aln -t ${NUM_T} -f results/aln/PE/${FILE_NAME}_R2_${PRE_REF}.sai  genome/mouse/${PRE_REF} rawdata/PE/${FILE_NAME}_R2.fastq.gz 1>> results/logs/PE/${LOG} 2>> results/logs/PE/${LOG}

#SAI2SAM

bwa sampe -r "@RG\tID:${ID}\tLB:${LB}\tSM:${SM}\tPL:${PL}\tCN:${CN}\tPU:${PU}" -P genome/mouse/${PRE_REF} results/aln/PE/${FILE_NAME}_R1_${PRE_REF}.sai results/aln/PE/${FILE_NAME}_R2_${PRE_REF}.sai rawdata/PE/${FILE_NAME}_R1.fastq.gz rawdata/PE/${FILE_NAME}_R2.fastq.gz -f results/aln/PE/${FILE_NAME}_${PRE_REF}.sam 1>> results/logs/PE/${LOG} 2>> results/logs/PE/${LOG} 
